...
---
= 使用Oracle Linux 7.9搭配NetApp ONTAP 功能
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
您可以使用「multiPath -ll」命令來驗證ONTAP 您的各個LUN的設定。下列各節提供對應至非ASA和ASA 對象的LUN的多重路徑輸出範例。
...
Oracle Linux 7.9作業系統經過編譯、可辨識ONTAP 出各種不完整的LUN、並自動正確設定所有組態參數。要啟動多重路徑精靈、必須存在「multiPath.conf」檔案、但您可以使用「Touch /etc/multipath.conf」命令來建立空白的零位元組檔案
...
NOTE: 若要設定Oracle Linux 7.9 RedHat Enterprise核心（RHCK）、請使用 link:hu_rhel_79.html#recommended-settings["建議設定"] 適用於Red Hat Enterprise Linux（RHEL）7.9。
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1328944["1328943."^] | 在使用Emulex LPe32002（32G）主機進行儲存容錯移轉期間、發現I/O延遲 | 在Oracle Linux 7.8核心（5.4.17-2011.1.2.el8uek.x86_64）與QLogic LPe32002 32G HBA上執行儲存容錯移轉作業期間、I/O作業可能無法繼續執行現有的作用中路徑。如果在儲存容錯移轉期間、由於無法繼續通過現有的作用中路徑而導致I/O進度停止、則I/O作業可能無法繼續、導致I/O延遲。只有在儲存容錯移轉還原作業完成之後、I/O作業才會恢復。 | link:https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=17319["17319"^] 
|===

NOTE: 如需Oracle Linux（Red Hat相容核心）已知問題、請參閱 link:hu_rhel_79.html#known-problems-and-limitations["已知問題"] 適用於Red Hat Enterprise Linux（RHEL）7.9。
