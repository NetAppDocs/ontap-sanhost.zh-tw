...
---
= 使用Oracle Linux 8.4搭配NetApp ONTAP 功能
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
Oracle Linux 8.4作業系統經過編譯、可辨識ONTAP 出各種不完整的LUN、並自動針對ASA 各種版本設定正確的所有組態參數。要啟動多重路徑精靈、必須存在「multiPath.conf」檔案、但您可以使用「Touch /etc/multipath.conf」命令來建立空白的零位元組檔案
...
NOTE: 若要設定Oracle Linux 8.4 RedHat Enterprise核心（RHCK）、請使用 link:hu_rhel_84.html#recommended-settings["建議設定"] 適用於Red Hat Enterprise Linux（RHEL）8.4。
