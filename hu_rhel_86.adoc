...
summary: 在 RHEL 8.6 主機上安裝 Linux Host Utilities 以使用 ONTAP LUN 管理 FCP 和 iSCSI 協定操作。 
---
= 使用 ONTAP 儲存設備為 FCP 和 iSCSI 設定 RHEL 8.6
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux 主機公用程式軟體可為連接至 ONTAP 儲存設備的 Linux 主機提供管理與診斷工具。在 Red Hat Enterprise Linux （ RHEL ） 8.6 主機上安裝 Linux 主機公用程式時，您可以使用主機公用程式來協助管理使用 ONTAP LUN 的 FCP 和 iSCSI 傳輸協定作業。
...
您可以搭配 RHEL 8.6 使用多重路徑來管理 ONTAP LUN 。
