...
主機公用程式支援不同的Windows主機組態、傳輸協定和多路徑選項。如需詳細資訊、請參閱 https://["NetApp 互通性對照表工具"^]。
...
. 驗證纜線
+
請參閱 https://["SAN組態指南"^] 適用於ONTAP 您的版本的 https://["E系列硬體佈線指南"^] 以取得詳細的纜線和組態資訊。
...
. 如需使用此功能、請將交換器分區為WWPN。ONTAP請務必使用LIF的WWPN、而非儲存控制器上的實體連接埠。如需詳細資訊、請參閱 https://["SAN組態指南"^]。
...
|===
.相關資訊
請參閱 https://["Microsoft文件"^] 以取得登錄參數詳細資料。
...
.相關資訊
請參閱 https://["Emulex"^] 或 https://["QLogic"^] 網站以深入瞭解逾時參數。
...
* 您必須檢閱中的疑難排解資訊 https://["SAN管理指南"^] 適用於ONTAP 您的版本的
* 您必須搜尋 https://["線上錯誤"^] 針對最近發現的問題。
