...
在開始故障排除之前，請確認您執行的配置符合以下要求： https://mysupport.netapp.com/matrix/#welcome["互通性對照表工具"^] 然後按照說明進行下一步操作，以調試任何主機端問題。


NOTE: 故障排除說明不適用於 AIX、ESXi、Proxmox 和 Windows 主機。
...
如果問題仍然存在，請收集以下文件和命令輸出，並聯絡 link:https://mysupport.netapp.com["NetApp支援"^] 進行進一步排查：
