...
summary: 如何為 RHEL 9.x 設定 NVMe-oF 主機（使用ONTAP） 
---
= 配置 RHEL 9.x 以使用 NVMe-oF 和ONTAP存儲
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 RHEL 9.x 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rhel-supported-features.html["RHEL ONTAP支援與功能"]。

NVMe-oF 與 RHEL 9.x 有以下已知限制：
...
.步驟
. 在伺服器上安裝 RHEL 9.x。安裝完成後，請確認您執行的是所需的 RHEL 9.x 核心：
+
[source, cli]
----
uname -r
...
.RHEL 9.x 版本中 iopolicy 的變更
* 從 RHEL 9.7 開始，預設的 iopolicy 是 queue-depth。
* 在 RHEL 9.6 中，round-robin 是預設的 iopolicy，但 queue-depth 可作為可設定的選項。
* 在 RHEL 9.6 之前，round-robin 是唯一支援的 NVMe-oF iopolicy。


如果您使用的是 RHEL 9.6 並且想要將 iopolicy 變更為  `queue-depth`，請依照下列方式修改 udev 規則檔案：
...
支援透過 NVMe/TCP 在 RHEL 9.x 主機和ONTAP控制器之間進行安全的帶內身份驗證。
