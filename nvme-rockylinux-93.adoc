...
summary: 如何使用 ONTAP 為 Rocky Linux 9.3 設定 NVMe-oF 主機 
---
= 使用ONTAP儲存為 NVMe-oF 設定 Rocky Linux 9.3
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 Rocky Linux 9.3 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支援與功能"]。

*帶有 Rocky Linux 9.3 的 NVMe-oF 有以下已知限制：*
...
.步驟
. 在伺服器上安裝 Rocky Linux 9.3。安裝完成後，請驗證您是否正在執行所需的 Rocky Linux 9.3 核心：
+
[source, cli]
----
uname -r
