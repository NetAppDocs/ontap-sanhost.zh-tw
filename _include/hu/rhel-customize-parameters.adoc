...
您應該檢查 `/etc/multipath.conf`檔案，尤其是在預設值區段中，以瞭解可能會覆寫的設定<<multipath-parameter-settings,多重路徑參數的預設設定>>。

例如，如果您的主機已連接至其他廠商的 LUN ，且任何多重路徑參數設定都會被覆寫，則您需要在檔案中稍後新增特定適用於 ONTAP LUN 的節點來修正這些設定 `multipath.conf`。如果您不這麼做， ONTAP LUN 可能無法如預期般運作。
