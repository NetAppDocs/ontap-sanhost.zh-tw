...
---
= 搭配 ONTAP 使用 Windows Server 2019
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定，將 Windows 伺服器 2019 與 ONTAP 一起設定，以便與 ONTAP 儲存設備搭配運作。
...
如果您的 Windows 主機有多個路徑通往儲存系統，則需要安裝 Microsoft 多重路徑 I/O （ MPIO ）軟體，並啟用多重路徑。當您選取 MPIO 支援時， Windows 主機公用程式會啟用 Windows Server 2019 中的 MPIO 功能。如果您未使用 Windows 主機公用程式，則需要手動啟用多重路徑功能。
