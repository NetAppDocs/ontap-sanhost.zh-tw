...
summary: 說明如何搭配ONTAP 使用Oracle Linux 7.3搭配使用功能 
---
= 將 Oracle Linux 7.3 與 ONTAP 搭配使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 Oracle Linux 7.3 與 ONTAP 設定為目標。
...
.您需要的產品
如果您決定使用SAN開機、則組態必須支援SAN開機。您可以使用 https://mysupport.netapp.com/matrix/imt.jsp?components=77396;&solution=1&isHWU&src=IMT["NetApp 互通性對照表工具"^] 驗證是否ONTAP 支援您的作業系統、HBA、HBA韌體和HBA開機BIOS及BIOS版本。
...
對於Oracle Linux 7.3、/etc/multipath.conf檔案必須存在、但您不需要對檔案進行特定變更。Oracle Linux 7.3的所有設定都經過編譯、可辨識及正確管理ONTAP 各種LUN。
...
Oracle Linux 7.3 作業系統的編譯是為了識別 ONTAP LUN 、並自動正確設定所有組態參數。該 `multipath.conf`檔案必須存在、多重路徑常駐程式才能啟動。如果此檔案不存在、您可以使用下列命令來建立空的零位元組檔案：
...
NOTE: 若要設定 Oracle Linux 7.3 Red Hat Enterprise 核心（ RCK ）、請使用link:hu_rhel_73.html#recommended-settings["建議設定"]適用於 Red Hat Enterprise Linux （ RHEL ） 7.3 的。



== 已知問題

使用 ONTAP 版本的 Oracle Linux 7.3 沒有已知問題。
