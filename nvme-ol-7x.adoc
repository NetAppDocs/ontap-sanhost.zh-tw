...
summary: 為 Oracle Linux 7x 和ONTAP設定 VMe/FC 主機配置，並提供範例 
---
...
了解如何為 Oracle Linux 7x 配置 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-ol-supported-features.html["Oracle Linux ONTAP支援與功能"]。
...
.步驟
. 在伺服器上安裝Oracle Linux 7x。安裝完成後，請確認您執行的是指定的 Oracle Linux 7x 核心。
+
[source, cli]
----
uname -r
...
----
. 在 Oracle Linux 7x 主機上，檢查 `hostnqn`字串 `/etc/nvme/hostnqn`：
+
[source, cli]
----
