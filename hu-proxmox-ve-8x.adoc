...
summary: 在 Proxmox VE 8.x 主機上安裝 Linux 主機實用程序，以管理ONTAP LUN 的 FCP 和 iSCSI 協定操作。 
---
= 配置 Proxmox VE 8.x 以支援 FCP 和 iSCSI 以及ONTAP存儲
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
配置 Proxmox VE 8.x 以實現多路徑，並為與ONTAP儲存的 FCP 和 iSCSI 協定操作設定特定的參數和設定。

FCP 和 iSCSI 與 Proxmox VE 8.x 有以下已知限制：

* Linux 主機實用程式不支援 Proxmox VE 8.x 作業系統。
* 不支援 SAN 啟動配置。
...
您可以使用 Proxmox VE 8.x 的多路徑功能來管理ONTAP LUN。
