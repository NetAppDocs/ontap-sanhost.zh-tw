...
---
= 適用於 ONTAP 的 RHEL 9.6 NVMe 主機組態
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
----
. 在 RHEL 9.6 主機上，請檢查 hostnqn 字串，網址為 `/etc/nvme/hostnqn`：
+
[source, cli]
----
...
使用 RHEL 9.6，  `nvmefc-boot-connections.service`和 `nvmf-autoconnect.service` NVMe/FC 中包含的啟動服務 `nvme-cli`軟體包會在系統啟動時自動啟用。系統啟動完成後，請驗證啟動服務是否已啟用。
