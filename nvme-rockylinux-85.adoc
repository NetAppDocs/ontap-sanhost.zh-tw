...
---
= 使用ONTAP儲存為 NVMe-oF 設定 Rocky Linux 8.5
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 Rocky Linux 8.5 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支援與功能"]。

*帶有 Rocky Linux 8.5 的 NVMe-oF 有以下已知限制：*

* 目前不支援使用 NVMe-oF 協定的 SAN 啟動。
* 在 Rocky Linux 8.5 中的 NVMe-oF 主機上，核心 NVMe 多路徑預設為停用；您必須手動啟用它。
...
.步驟
. 在伺服器上安裝 Rocky Linux 8.5。安裝完成後，驗證您正在運行所需的 Rocky Linux 8.5 核心：
+
[source, cli]
----
uname -r
