...
summary: 如何使用ONTAP為 RHEL 10.0 設定 NVMe-oF 主機 
---
= 使用ONTAP儲存為 NVMe-oF 設定 RHEL 10.0
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 RHEL 10.0 設定 NVMe over Fabrics (NVMe-oF) 主機。有關更多支援和功能信息，請參閱link:hu-nvme-index.html["NVME-oF 概述"^]。

*RHEL 10.0 的 NVMe-oF 有以下已知限制：*
...
.步驟
. 在伺服器上安裝 RHEL 10.0。安裝完成後，請驗證您是否正在執行所需的 RHEL 10.0 核心：
+
[source, cli]
----
uname -r
