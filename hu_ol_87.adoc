...
---
= 搭配 NetApp ONTAP 使用 Oracle Linux 8.7
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
NOTE: 請勿使用過多的路徑來通往單一LUN。您應該不需要超過四個路徑。在儲存設備故障期間、可能會有八個以上的路徑發生問題。
...
NOTE: 若要設定 Oracle Linux 8.7 RedHat Enterprise Kernel （ RHCK ）、請使用 link:hu_rhel_87.html#recommended-settings["建議設定"] 適用於 Red Hat Enterprise Linux （ RHEL ） 8.7 。



== KVM 設定

您也可以使用建議的設定來設定核心型虛擬機器（ KVM ）。當 LUN 對應至 Hypervisor 時、無需進行任何變更即可設定 KVM 。



== 已知問題

Oracle Linux 8.7 沒有已知問題。
...
自動儲存管理（ ASM ）鏡射可能需要變更 Linux 多重路徑設定、以允許 ASM 識別問題並切換至替代的故障群組。大部分關於「不完整」的ASM組態ONTAP 都使用外部備援、這表示資料保護是由外部陣列提供、而ASM不會鏡射資料。某些站台使用具有一般備援的ASM來提供雙向鏡像、通常是跨不同站台。如需詳細資訊、請參閱 ONTAP 上的 Oracle 資料庫。
