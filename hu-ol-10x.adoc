...
summary: 在 Oracle Linux 10.x 主機上安裝 Linux Host Utilities，以管理 ONTAP LUN 的 FCP 和 iSCSI 協定操作。 
---
= 使用 ONTAP 儲存設備為 Oracle Linux 10.x 設定 FCP 和 iSCSI
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 軟體為連接到 ONTAP 儲存設備的 Linux 主機提供管理和診斷工具。在 Oracle Linux 10.x 主機上安裝 Linux Host Utilities 後，您可以使用 Host Utilities 來協助管理 ONTAP LUN 的 FCP 和 iSCSI 協定作業。
...
您可以使用 Oracle Linux 10.x 的多路徑功能來管理 ONTAP LUN。
