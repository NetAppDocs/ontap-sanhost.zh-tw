...
summary: 說明如何搭配ONTAP 使用Oracle Linux 8.3搭配使用功能 
---
= 搭配 ONTAP 使用 Oracle Linux 8.3
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 ONTAP 設定為目標的 Oracle Linux 8.3 。
...
對於Oracle Linux（OL）8.3、/etc/multipath.conf檔案必須存在、但您不需要對檔案進行特定變更。OL 8.3是以所有必要設定來編譯、以辨識及正確管理ONTAP 各種LUN。
...
Oracle Linux 8.3作業系統已經過編譯、可辨識ONTAP 出各種不完整的LUN、並自動針對ASA 各種版本設定正確的所有組態參數。
...
NOTE: 若要設定Oracle Linux 8.3 Red Hat Enterprise核心（RHCK）、請使用 link:hu_rhel_83.html#recommended-settings["建議設定"] 適用於Red Hat Enterprise Linux（RHEL）8.3。



== 已知問題

使用 ONTAP 版本的 Oracle Linux 8.3 沒有已知問題。


NOTE: 如需Oracle Linux（Red Hat相容核心）已知問題、請參閱 link:hu_rhel_83.html#known-problems-and-limitations["已知問題"] 相關Red Hat Enterprise Linux版本文件中的一節。
