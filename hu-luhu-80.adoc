...
.關於這項工作
安裝 Linux Host Utilities 時，不會變更主機上的任何設定或行為。
...
NOTE: Linux Host Utilities 的安裝相依於 libHBAAPI.so 程式庫。即使主機僅用於 iSCSI 專用組態，也需要安裝上述其中一個套件。或者，您可以使用作業系統層級的工具進行 LUN 和路徑管理。
