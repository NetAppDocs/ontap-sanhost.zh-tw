...
summary: 說明如何搭配ONTAP 使用Oracle Linux 8.6. 
---
= 將 Oracle Linux 8.6 與 ONTAP 搭配使用
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 Oracle Linux 8.6 與 ONTAP 設定為目標。
...
NOTE: 若要設定 Oracle Linux 8.6 Red Hat Enterprise 核心（ RHHCK ）、請使用link:hu_rhel_86.html#recommended-settings["建議設定"]適用於 Red Hat Enterprise Linux （ RHEL ） 8.6 的。
...
使用 ONTAP 版本的 Oracle Linux 8.6 沒有已知問題。


NOTE: 如需Oracle Linux（Red Hat相容核心）已知問題、請參閱 link:hu_rhel_86.html#known-problems-and-limitations["已知問題"] 相關Red Hat Enterprise Linux版本文件中的一節。
