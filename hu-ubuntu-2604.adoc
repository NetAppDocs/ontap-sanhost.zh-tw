...
summary: 在 Ubuntu 26.04 主機上安裝 Linux Host Utilities，以管理 ONTAP LUN 的 FCP 和 iSCSI 協定操作。 
---
= 配置 Ubuntu 26.04 以支援 FCP 和 iSCSI，並使用 ONTAP 儲存設備
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 軟體為連接到 ONTAP 儲存設備的 Linux 主機提供管理和診斷工具。在 Ubuntu 26.04 主機上安裝 Linux Host Utilities 後，您可以使用 Host Utilities 來協助管理 ONTAP LUN 的 FCP 和 iSCSI 協定操作。
...
您可以使用 Ubuntu 26.04 的多路徑功能來管理 ONTAP LUN。
