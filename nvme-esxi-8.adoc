...
* 從 ONTAP 9.16.1 開始，預設會為所有新建立的 NVMe 命名空間啟用空間分配。
* 從 ONTAP 9.9.1 P3 開始，ESXi 8 支援 NVMe/FC 協定。
* 從 ONTAP 9.10.1 開始，ONTAP 支援 NVMe/TCP 協定。
...
| link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1420654["1420654"^] | 當使用 NVMe/FC 協定且 ONTAP 版本為 9.9.1 時，ONTAP 節點無法運作 | ONTAP 9.9.1 引入了對 NVMe "abort" 指令的支援。當 ONTAP 收到用於中止正在等待其夥伴指令的 NVMe 融合指令的 "abort" 指令時，ONTAP 節點會發生中斷。此問題僅在使用 NVMe 融合指令（例如 ESX）和光纖通道（FC）傳輸的主機上出現。 


| link:https://jira.ngage.netapp.com/browse/CONTAPEXT-1126["CONTAPEXT-1126"^] | 當使用 vNVMe 介面卡的 Linux VM 遇到長時間的 All Paths Down （ APD ）視窗時、就會發生 I/O 錯誤  a| 
執行 vSphere 8.x 及更高版本且使用虛擬 NVMe（vNVMe）適配器的 Linux VM 會遇到 I/O 錯誤，因為 vNVMe 重試操作預設為停用。為了避免在全路徑故障（APD）或高 I/O 負載期間，執行舊核心的 Linux VM 出現中斷，VMware 引入了一個可調參數 "VSCSIDisableNvmeRetry" 來停用 vNVMe 重試操作。
