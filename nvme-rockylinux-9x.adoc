...
summary: 如何為 Rocky Linux 9.x 設定 NVMe-oF 主機和ONTAP 
---
= 配置 Rocky Linux 9.x 以支援 NVMe-oF 和ONTAP存儲
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 Rocky Linux 9.x 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支援與功能"]。

NVMe-oF 與 Rocky Linux 9.x 有以下已知限制：
...
== 步驟 2：安裝 Rocky Linux 和 NVMe 軟體並驗證您的配置

若要為 NVMe-oF 設定主機，您需要安裝主機和 NVMe 軟體包，啟用多路徑，並驗證主機 NQN 設定。

.步驟
. 在伺服器上安裝 Rocky Linux 9.x。安裝完成後，請確認您執行的是所需的 Rocky Linux 9.x 核心：
+
[source, cli]
----
uname -r
...
Rocky Linux 9x 主機和ONTAP控制器之間透過 NVMe/TCP 支援安全的帶內身份驗證。
...
----
. 為 Rocky Linux 9.x 主機產生 dhchap 金鑰。
+
