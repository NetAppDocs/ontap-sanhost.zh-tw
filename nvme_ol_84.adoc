...
| 1517321 | Oracle Linux 8.4 NVMe主機會建立重複的持續探索控制器 | 在Oracle Linux 8.4 NVMe over Fabrics（NVMe / of）主機上、您可以使用「NVMe探索-p」命令來建立持續探索控制器（PD）。使用此命令時、每個啟動器目標組合只能建立一個PDC。不過、如果您執行ONTAP 的是採用NVMe的主機執行的支援功能、則每次執行「NVMe探索-p」時、都會建立一個重複的資料中心。這會導致主機和目標上的資源使用不必要。 | 2087000 
|===
