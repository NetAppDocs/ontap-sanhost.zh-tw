...
---
= 使用ONTAP儲存為 NVMe-oF 設定 Rocky Linux 9.0
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 Rocky Linux 9.0 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支援與功能"]。

*帶有 Rocky Linux 9.0 的 NVMe-oF 具有以下已知限制：*
...
.步驟
. 在伺服器上安裝Rocky Linux 9.0。安裝完成後，請驗證您是否正在執行所需的 Rocky Linux 9.0 核心：
+
[source, cli]
----
uname -r
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Rocky Linux 9.0 NVMe-oF 主機建立重複的持久發現控制器 | 在 NVMe-oF 主機上，您可以使用「nvme discover -p」指令建立持久發現控制器 (PDC)。使用此命令時，每個啟動器-目標組合只能建立一個 PDC。但是，如果您在 NVMe-oF 主機上執行ONTAP 9.10.1 和 Rocky Linux 9.0，則每次執行「nvme discover -p」時都會建立重複的 PDC。這會導致主機和目標上資源的不必要使用。 
|===