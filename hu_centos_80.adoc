...
summary: 在 CentOS 8.0 主機上安裝 Linux Host Utilities 以管理使用ONTAP LUN 的 FCP 和 iSCSI 協定操作。 
---
= 為 ONTAP 儲存配置 CentOS 8.0 的 FCP 和 iSCSI
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux 主機公用程式軟體可為連接至 ONTAP 儲存設備的 Linux 主機提供管理與診斷工具。在 CentOS 8.0 主機上安裝 Linux Host Utilities 時，您可以使用 Host Utilities 來協助您管理 ONTAP LUN 的 FCP 和 iSCSI 協定操作。
...
您可以使用 CentOS 8.0 的多路徑來管理 ONTAP LUN。
...
對於具有 ONTAP 儲存的 FCP 和 iSCSI，CentOS 8.0 沒有已知問題。

有關 CentOS Red Hat 相容核心的已知問題，請參閱link:hu_rhel_80.html#known-problems-and-limitations["已知問題"]適用於 Red Hat Enterprise Linux (RHEL) 8.0。
