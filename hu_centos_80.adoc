...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 CentOS 8.0 與 ONTAP 設定為目標。
...
CentOS 8.0 作業系統會編譯為識別 ONTAP LUN 、並自動為 ASA 和非 ASA 組態正確設定所有組態參數。該 `multipath.conf`檔案必須存在、多重路徑常駐程式才能啟動。如果此檔案不存在，您可以使用命令建立空白的零位元組檔案 `touch /etc/multipath.conf`
