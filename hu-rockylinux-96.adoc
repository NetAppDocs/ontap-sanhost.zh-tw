...
summary: 在 Rocky Linux 9.6 主機上安裝 Linux Host Utilities 以管理使用 ONTAP LUN 的 FCP 和 iSCSI 協定操作。 
---
= 使用 ONTAP 儲存為 FCP 和 iSCSI 設定 Rocky Linux 9.6
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux 主機公用程式軟體可為連接至 ONTAP 儲存設備的 Linux 主機提供管理與診斷工具。在 Rocky Linux 9.6 主機上安裝 Linux Host Utilities 時，您可以使用 Host Utilities 來協助您管理 ONTAP LUN 的 FCP 和 iSCSI 協定操作。
...
您可以使用 Rocky Linux 9.6 的多路徑來管理 ONTAP LUN。
