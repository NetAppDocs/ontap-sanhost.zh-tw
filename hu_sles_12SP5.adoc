...
summary: 在 SUSE Linux Enterprise Server 12 SP5 上安裝 Linux Host Utilities 以使用 ONTAP LUN 管理 FCP 和 iSCSI 協定操作。 
---
= 設定適用於 FCP 的 SUSE Linux Enterprise Server 12 SP5 ，以及使用 ONTAP 儲存設備的 iSCSI
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux 主機公用程式軟體可為連接至 ONTAP 儲存設備的 Linux 主機提供管理與診斷工具。在 SUSE Linux Enterprise Server 12 SP5 主機上安裝 Linux 主機公用程式時，您可以使用主機公用程式來協助管理使用 ONTAP LUN 的 FCP 和 iSCSI 傳輸協定作業。
...
您可以在 SUSE Linux Enterprise Server 12 SP5 中使用多重路徑來管理 ONTAP LUN 。
...
具有 ONTAP 儲存版本的 SUSE Linux Enterprise Server 12 SP5 有下列已知問題：
