...
---
= 將CentOS 8.3搭配NetApp ONTAP 功能使用
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
CentOS 8.3作業系統經過編譯、可辨識ONTAP 出各種無法辨識的LUN、並自動針對ASA 各種版本設定正確的所有組態參數。要啟動多重路徑精靈、必須存在「multiPath.conf」檔案、但您可以使用「Touch /etc/multipath.conf」命令來建立空白的零位元組檔案
