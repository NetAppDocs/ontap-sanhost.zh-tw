...
---
= 使用 SUSE Linux Enterprise Server 15 SP3 搭配 ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 SUSE Linux Enterprise Server 15 SP3 與 ONTAP 設定為目標。
...
對於SUSE Linux Enterprise Server 15 SP3、必須存在「/etc/multipath.conf」檔案、但您不需要對該檔案進行特定變更。SUSE Linux Enterprise Server 15 SP3的所有設定都已經過編譯、可辨識及正確管理ONTAP 各種LUN。
...
SUSE Linux Enterprise Server 15 SP3作業系統經過編譯、可辨識ONTAP 出各種不完整的LUN、並自動針對ASA 各個版本的顯示和非ASA組態、正確設定所有組態參數。
...
SUSE Linux Enterprise Server 15 SP3 with ONTAP 版本並無已知問題。
