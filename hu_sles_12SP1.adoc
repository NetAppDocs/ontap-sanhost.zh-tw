...
---
= 使用SUSE Linux Enterprise Server 12 SP1搭配NetApp ONTAP 功能
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
SUSE Linux Enterprise Server 12 SP1作業系統經過編譯、可辨識ONTAP 出各種LUN、並自動正確設定所有組態參數。要啟動多重路徑精靈、必須存在「multiPath.conf」檔案、但您可以使用「Touch /etc/multipath.conf」命令來建立空白的零位元組檔案
...
SUSE Linux Enterprise Server 12 SP1沒有已知問題。
