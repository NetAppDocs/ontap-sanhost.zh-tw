...
summary: 您可以在執行 Windows Server 2012 R2 的主機上設定 NVMe over Fibre Channel （ NVMe / FC ），以便與 ONTAP LUN 一起運作。 
---
= 使用適用於 ONTAP 的 NVMe / FC 設定 Windows Server 2012 R2
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以在執行 Windows Server 2012 R2 的主機上設定 NVMe over Fibre Channel （ NVMe / FC ），以便與 ONTAP LUN 一起運作。

.關於這項工作
您可以在 Windows 2012 R2 的 NVMe / FC 主機組態中使用下列支援。在開始設定程序之前，您也應該先檢閱已知的限制。
...
* 可用支援：
+
從 ONTAP 9.7 開始， Windows Server 2012 R2 支援 NVMe / FC 。
