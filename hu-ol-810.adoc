...
summary: 說明如何搭配 ONTAP 使用 Oracle Linux 8.10 
---
...
.開始之前
使用link:https://mysupport.netapp.com/matrix/imt.jsp?components=91863;&solution=1&isHWU&src=IMT["NetApp 互通性對照表工具"^]驗證是否支援您的 OS ， HBA ， HBA 韌體， HBA 開機 BIOS 和 ONTAP 版本。
...
| 廠商 | NetApp 
|===
下列範例示範如何修正覆寫的預設值。在這種情況下，檔案會 `multipath.conf`定義與 `no_path_retry` ONTAP LUN 不相容的值 `path_checker`。如果因為其他SAN陣列仍連接至主機而無法移除這些參數、則可針對ONTAP 具有裝置例項的LUN、特別修正這些參數。
...
NOTE: 若要設定 Oracle Linux 8.10 RedHat Enterprise 核心、請使用link:hu_rhel_810.html#recommended-settings["建議設定"]適用於 Red Hat Enterprise Linux （ RHEL ） 8.10 的。



== KVM 設定

您也可以使用<<recommended_settings_ol_810,建議設定>>來設定核心型虛擬機器（ KVM ）。由於 LUN 已對應至 Hypervisor ，因此不需要變更即可設定 KVM 。



== 已知問題

ONTAP 版本的 Oracle Linux 8.10 沒有已知問題。
