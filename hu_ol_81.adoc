...
summary: 說明如何搭配ONTAP 使用Oracle Linux 8.1搭配使用功能 
---
= 搭配 ONTAP 使用 Oracle Linux 8.1
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 Oracle Linux 8.1 與 ONTAP 設定為目標。
...
對於Oracle Linux 8.1、/etc/multipath.conf檔案必須存在、但您不需要對檔案進行特定變更。Oracle Linux 8.1的所有設定都經過編譯、可辨識及正確管理ONTAP 各種LUN。
...
Oracle Linux 8.1作業系統經過編譯、可辨識ONTAP 出各種不完整的LUN、並自動正確設定所有組態參數。
...
NOTE: 若要設定 Oracle Linux 8.1 Red Hat Enterprise 核心（ RCK ）、請使用link:hu_rhel_81.html#recommended-settings["建議設定"]適用於 Red Hat Enterprise Linux （ RHEL ） 8.1 的。



== 已知問題

使用 ONTAP 版本的 Oracle Linux 8.1 沒有已知問題。


NOTE: 如需Oracle Linux（Red Hat相容核心）已知問題、請參閱 link:hu_rhel_81.html#known-problems-and-limitations["已知問題"] 適用於Red Hat Enterprise Linux（RHEL）8.1。
