...
---
= 使用ONTAP儲存為 NVMe-oF 設定 RHEL 8.10
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 RHEL 8.10 設定 NVMe over Fabrics (NVMe-oF) 主機。有關更多支援和功能信息，請參閱link:hu-nvme-index.html["NVME-oF 概述"^]。

*RHEL 8.10 的 NVMe-oF 有以下已知限制：*

* 目前不支援使用 NVMe-oF 協定的 SAN 啟動。
* 預設情況下，RHEL 8.10 中的 NVMe-oF 主機上會停用核心 NVMe 多路徑；您必須手動啟用它。
...
.步驟
. 在伺服器上安裝 RHEL 8.10。安裝完成後，請驗證您是否正在執行所需的 RHEL 8.10 核心：
+
[source, cli]
----
uname -r
...
----
. 在 RHEL 8.10 主機上，檢查 `hostnqn`字串位於 `/etc/nvme/hostnqn`：
+
[source, cli]
----
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | RHEL 8.10 NVMe-oF 主機建立重複的持久發現控制器 (PDC) | 在 NVMe-oF 主機上，您可以使用「nvme discover -p」指令建立 PDC。使用此命令時，每個啟動器-目標組合只能建立一個 PDC。但是，如果您在 NVMe-oF 主機上執行 RHEL 8.10，則每次執行「nvme discover -p」時都會建立重複的 PDC。這會導致主機和目標上資源的不必要使用。 
|===