...
summary: 如何為 RHEL 8.x 設定 NVMe-oF 主機（使用ONTAP） 
---
= 配置 RHEL 8.x 以使用 NVMe-oF 和ONTAP存儲
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 RHEL 8.x 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rhel-supported-features.html["RHEL ONTAP支援與功能"]。

NVMe-oF 與 RHEL 8.x 有以下已知限制：

* 目前不支援使用 NVMe-oF 協定的 SAN 啟動。
* 在 RHEL 8.x 的 NVMe-oF 主機上，核心內 NVMe 多路徑預設為停用；您必須手動啟用它。
...
.步驟
. 在伺服器上安裝 RHEL 8.x。安裝完成後，請確認您執行的是所需的 RHEL 8.x 核心：
+
[source, cli]
----
uname -r
...
----
. 在 RHEL 8.x 主機上，檢查 `hostnqn`字串 `/etc/nvme/hostnqn`：
+
[source, cli]
----
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | RHEL 8.x NVMe-oF 主機建立重複的持久發現控制器 (PDC) | 在 NVMe-oF 主機上，可以使用「nvme discover -p」指令建立 PDC。使用此指令時，每個發起方-目標組合只能建立一個 PDC。但是，如果您在 NVMe-oF 主機上執行 RHEL 8.x，則每次執行「nvme discover -p」時都會建立重複的 PDC。這會導致主機和目標裝置上的資源被不必要地消耗掉。 
|===