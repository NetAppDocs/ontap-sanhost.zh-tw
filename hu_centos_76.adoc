...
summary: 在 CentOS 7.6 主機上安裝 Linux Host Utilities 以使用ONTAP LUN 管理 FCP 和 iSCSI 協定操作。 
---
= 為 ONTAP 儲存配置 CentOS 7.6 的 FCP 和 iSCSI
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux 主機公用程式軟體可為連接至 ONTAP 儲存設備的 Linux 主機提供管理與診斷工具。在 CentOS 7.6 主機上安裝 Linux Host Utilities 時，您可以使用 Host Utilities 來協助您管理 ONTAP LUN 的 FCP 和 iSCSI 協定操作。
...
您可以使用 CentOS 7.6 的多路徑來管理 ONTAP LUN。
...
帶有 ONTAP 儲存的 CentOS 7.6 主機有以下已知問題：
