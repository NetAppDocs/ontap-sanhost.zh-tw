...
summary: 在 Windows Server 2022 主機上安裝 Windows Host Utilities，以協助您管理ONTAP LUN 的 FCP 和 iSCSI 協定作業。 
---
= 配置 Windows Server 2022 以支援 FCP 和 iSCSI 以及ONTAP存儲
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Windows 主機公用程式可讓您將 Windows 主機連接到NetApp SAN 上的虛擬磁碟 (LUN)。在 Windows Server 2022 主機上安裝 Windows 主機實用程序，以協助您管理ONTAP LUN 的 FCP 和 iSCSI 協定作業。
