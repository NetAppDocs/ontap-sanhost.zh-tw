...
了解如何為 Red Hat Enterprise Linux (RHEL) 9.6 設定 NVMe over Fabrics (NVMe-oF) 主機。有關更多支援和功能信息，請參閱link:hu-nvme-index.html["NVME-oF 概述"^]。

*RHEL 9.6 的 NVMe-oF 有以下已知限制：*
...
.步驟
. 在伺服器上安裝 RHEL 9.6。安裝完成後，請驗證您是否正在執行所需的 RHEL 9.6 核心：
+
[source, cli]
----
uname -r
...
RHEL 9.6 將 NVMe-oF 的預設 iopolicy 設定為 `round-robin`。如果你想將 iopolicy 更改為 `queue-depth`，修改udev規則文件如下：
...
----
. 驗證子系統的目前 iopolicy。將 <subsystem> 替換為適當的名稱，例如， `nvme-subsys0` 。
+
[source, cli]
----
...
從ONTAP 9.12.1 開始，RHEL 10.0 主機和ONTAP控制器之間透過 NVMe/TCP 支援安全帶內身份驗證。
