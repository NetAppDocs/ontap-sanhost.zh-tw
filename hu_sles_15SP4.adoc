...
下表顯示ONTAP 了適用於整個過程的關鍵多路徑參數、以及所需的值。如果主機連接至其他廠商的 LUN 、且這些參數中的任何一個被覆寫、則需要稍後在中的節段加以修正 `multipath.conf` 適用於 ONTAP LUN 。如果沒有這麼做、ONTAP 則可能無法如預期般運作。只有在完全瞭解影響的情況下、才應在諮詢NetApp和/或作業系統廠商的意見下覆寫這些預設值。
