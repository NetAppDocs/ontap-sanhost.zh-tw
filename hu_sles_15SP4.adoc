...
第一次建立此檔案時、可能需要啟用及啟動多重路徑服務。
 ...
 `multipath.conf`除非您有不想要多重路徑管理的裝置、或是現有的設定會覆寫預設值、否則不需要直接將任何項目新增至檔案。

您可以將下列語法新增至「multiPath.conf」檔案、以排除不需要的裝置。

更換 `<DevId>` 使用 `WWID` 要排除的裝置字串。請使用下列命令來判斷 `WWID`：
...
----
. 新增 `WWID` 中黑名單的值 `/etc/multipath.conf` 檔案：
+
[listing]
----
blacklist {
...
下表顯示ONTAP 了適用於整個過程的關鍵多路徑參數、以及所需的值。如果主機連接至其他廠商的 LUN 、且這些參數中的任何一個被覆寫、則需要在 `multipath.conf`適用於 ONTAP LUN 的後續節段中加以修正。如果沒有這麼做、ONTAP 則可能無法如預期般運作。您只能在諮詢 NetApp 、作業系統廠商或兩者之後、以及完全瞭解影響時、才應覆寫這些預設值。
