...
summary: 如何為 Proxmox VE 9.x 配置 NVMe-oF 主機和ONTAP存儲 
---
= 配置 Proxmox VE 9.x 以支援 NVMe-oF 和ONTAP存儲
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Proxmox VE 9.x 主機支援基於光纖通道的 NVMe (NVMe/FC) 和基於 TCP 的 NVMe (NVMe/TCP) 協議，並支援非對稱命名空間存取 (ANA)。ANA 提供與 iSCSI 和 FCP 環境中的非對稱邏輯單元存取 (ALUA) 等效的多路徑功能。

了解如何為 Proxmox VE 9.x 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-proxmox-supported-features.html["ONTAP支援和功能"^]。

NVMe-oF 與 Proxmox VE 9.x 有以下已知限制：
...
.步驟
. 在伺服器上安裝 Proxmox VE 9.x。安裝完成後，請確認您執行的是所需的 Proxmox VE 9.x 核心：
+
[source, cli]
----
uname -r
...
從 Proxmox VE 9.0 開始，NVMe-oF 的預設 iopolicy 設定為 `queue-depth`。如果您想將 iopolicy 變更為 `round-robin`（該選項也可配置），請如下修改 udev rules 檔案：
...
----
. 修改規則，使  `queue-depth` 變更為  `round-robin`，如下列範例所示：
+
[listing]
----
...
----
. 請驗證子系統的目前 I/O 策略。將<subsystem>替換為您的子系統名稱，例如 nvme-subsys0。
+
[source, cli]
----
...
.NVMe / FC
--
以下範例輸出顯示了託管在雙節點 ONTAP 控制器上的命名空間，該命名空間適用於具有 NVMe/FC 的 AFF、FAS、ASA 或 ASA r2 系統。

.顯示 AFF、FAS 或 ASA 範例輸出
[%collapsible]
=====
[listing, subs="+quotes"]
----
