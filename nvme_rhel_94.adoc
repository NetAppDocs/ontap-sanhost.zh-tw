...
NVMe / TCP 沒有自動連線功能。因此、我們需要手動執行 NVMe / TCP 連線或全部連線功能、才能探索 NVMe / TCP 子系統和命名空間。
...
NOTE: 從 RHEL 9.4 開始、是 NVMe / TCP 的預設設定 `ctrl_loss_tmo` 逾時已關閉、這表示重試次數沒有限制（無限期重試）。因此、您不需要手動設定特定的 `ctrl_loss_tmo` 使用 NVMe 連線或 NVMe 連線全部命令（選項 -l ）時的逾時時間。在這種預設行為下、如果路徑故障、則 NVMe / TCP 控制器不會發生逾時、而且會無限期地保持連線。
