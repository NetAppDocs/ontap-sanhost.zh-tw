...
summary: 說明如何搭配ONTAP 使用Oracle Linux 7.1搭配使用 
---
= 將 Oracle Linux 7.1 與 ONTAP 搭配使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 Oracle Linux 7.1 與 ONTAP 設定為目標。
...
.您需要的產品
如果您決定使用SAN開機、則組態必須支援SAN開機。您可以使用 https://mysupport.netapp.com/matrix/imt.jsp?components=70671;&solution=1&isHWU&src=IMT["NetApp 互通性對照表工具"^] 驗證是否ONTAP 支援您的作業系統、HBA、HBA韌體和HBA開機BIOS及BIOS版本。
...
Oracle Linux 7.1支援Unbreakable Enterprise Kernel（UEK）R3和UEK R4。作業系統預設會以UEK R3核心開機。

.Oracle Linux 7.1 UEK R3組態
對於Oracle Linux 7.1 UEK R3、請建立一個空的multipath.conf檔案。Oracle Linux 7.1 UEK的設定預設會自動更新、且不會更新ALUA。若要啟用ALUA處理常式、請執行下列步驟：
...
.Oracle Linux 7.1 UEK R4組態
對於Oracle Linux 7.1 UEK R4、/etc/multipath.conf檔案必須存在、但您不需要對檔案進行特定變更。Oracle Linux 7.1的編譯包含所有必要設定、可辨識及正確管理ONTAP 各種LUN。您可以使用 `multipath -ll`命令來驗證 ONTAP LUN 的設定。應該有兩種路徑群組具有不同的優先順序。優先順序較高的路徑為主動 / 最佳化、這表示它們由集合所在的控制器提供服務。優先順序較低的路徑是作用中的、但未最佳化、因為它們是由不同的控制器提供服務。非最佳化路徑只有在最佳化路徑無法使用時才會使用。
...
Oracle Linux 7.1作業系統經過編譯、可辨識ONTAP 出各種不完整的LUN、並自動正確設定所有組態參數。
...
NOTE: 若要設定 Oracle Linux 7.1 Red Hat Enterprise 核心（ RCK ）、請使用link:hu_rhel_71.html#recommended-settings["建議設定"]適用於 Red Hat Enterprise Linux （ RHEL ） 7.1 的。



== 已知問題

使用 ONTAP 版本的 Oracle Linux 7.1 沒有已知問題。
