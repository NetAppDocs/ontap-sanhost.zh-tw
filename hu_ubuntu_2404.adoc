...
summary: 說明如何搭配 ONTAP 使用 Ubuntu 24.04 
---
= 搭配 ONTAP 使用 Ubuntu 24.04
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
NOTE: NetApp Linux Unified Host Utilities 軟體套件不適用於 Ubuntu 24.04 作業系統。
...
對於 Ubuntu 24.04 ， `/etc/multipath.conf`檔案必須存在，但您不需要變更檔案，因為 Ubuntu 24.04 是使用辨識及正確管理 ONTAP LUN 所需的設定進行編譯。
...
Ubuntu 24.04 作業系統是為了辨識 ONTAP LUN ，並針對 ASA 和非 ASA 組態，自動正確設定所有組態參數。此外，您可以使用下列建議設定，進一步最佳化主機組態的效能。
...
| 廠商 | NetApp 
|===
下列範例說明如何修正被覆寫的預設值。在這種情況下，檔案會 `multipath.conf`定義與 `no_path_retry` ONTAP LUN 不相容的值 `path_checker`。如果因為其他SAN陣列仍連接至主機而無法移除這些參數、則可針對ONTAP 具有裝置例項的LUN、特別修正這些參數。
...
使用 ONTAP 版本的 Ubuntu 24.04 沒有已知問題。
