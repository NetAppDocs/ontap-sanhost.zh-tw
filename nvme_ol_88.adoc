...
summary: 適用於 ONTAP 的 Oracle Linux 8.8 版 NVMe 型主機組態 
---
= 適用於 ONTAP 的 Oracle Linux 8.8 版 NVMe 型主機組態
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
[role="lead"]
Oracle Linux （ OL ） 8.8 支援 NVMe over Fabrics （ NVMe over Fabric 、 NVMe over Fibre Channel 、 NVMe / FC ）和其他傳輸、並支援非對稱命名空間存取（ ANA ）。在 NVMe 環境中、 ANA 等同於 iSCSI 和 FC 環境中的 ALUA 多重路徑、並以核心內建 NVMe 多重路徑來實作。

下列支援可用於 OL 8.8 搭配 ONTAP 的 NVMe 主機組態：
...
* 不支援NVMe的sanlun。因此、 OL 8.8 主機上不支援 NVMe 型主機公用程式。您可以仰賴原生內建的 NetApp 外掛程式 `nvme-cli` 適用於所有 NVMe 傳輸的套件。
...
* Oracle Linux 8.8 預設已啟用 NVMe 命名空間的核心內建 NVMe 多重路徑、因此不需要明確的設定。
...
您可以使用下列程序來驗證支援的最低 OL 8.8 軟體版本。

.步驟
. 在伺服器上安裝 OL 8.8 GA 。安裝完成後、請確認您正在執行指定的 OL 8.8 GA 核心。
+
[listing]
----
# uname -r
...
----
. 在 Oracle Linux 8.8 主機上、請檢查 `hostnqn` 字串at `/etc/nvme/hostnqn`：
+
[listing]
----
...
----
執行以重新啟動多路徑常駐程式 `systemctl restart multipathd` 讓新設定生效的命令。
...
--
.步驟
. OL 8.8 GA 核心隨附的原生收件匣 qla2xxx 驅動程式具有 ONTAP 支援所需的最新上游修正程式。確認您執行的是支援的介面卡驅動程式和韌體版本：
+
[listing]
----
...
OL 8.8 搭配 ONTAP 版本的 NVMe 主機組態有下列已知問題：
...
| 1517321 | Oracle Linux 8.8 NVMe 主機會建立重複的持續探索控制器 | 在 OL 8.8 NVMe 型主機上、持續探索控制器（ PDC ）是透過傳遞建立的 `-p` 選項 `nvme discover` 命令。對於指定的啟動器目標組合、每次啟動時、只會建立一個 PDC `nvme discover` 命令。不過、從 OL 8.x 開始、 NVMe 型主機會在每次呼叫時建立重複的 PDF `nvme discover` 命令 `-p` 選項。這會浪費主機和目標上的資源。 | https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=18118["18118."^] 
|===