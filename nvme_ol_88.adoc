...
--
.步驟
. OL 8.8 GA 核心隨附的原生收件匣 qla2xxx 驅動程式具有 ONTAP 支援所需的最新修正程式。確認您執行的是支援的介面卡驅動程式和韌體版本：
+
[listing]
----
...
| 1517321 | Oracle Linux 8.8 NVMe 主機會建立重複的 PDF | 在 OL 8.8 NVMe 型主機上、持續探索控制器（ PDC ）是透過傳遞建立的 `-p` 選項 `nvme discover` 命令。對於指定的啟動器目標組合、每次啟動時、只會建立一個 PDC `nvme discover` 命令。不過、從 OL 8.x 開始、 NVMe 型主機會在每次呼叫時建立重複的 PDF `nvme discover` 命令 `-p` 選項。這會浪費主機和目標上的資源。 | https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=18118["18118."^] 
|===