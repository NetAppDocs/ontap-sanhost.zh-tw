...
* 不支援NVMe的sanlun。因此、 OL 8.8 主機上不支援 NVMe 型主機公用程式。您可以仰賴原生 `nvme-cli`套件中隨附的 NetApp 外掛程式來執行所有 NVMe 傳輸。
...
--
OL 8.8 GA 核心中隨附的原生收件匣 qla2xxx 驅動程式具有最新的修正程式。這些修正對於 ONTAP 支援至關重要。
