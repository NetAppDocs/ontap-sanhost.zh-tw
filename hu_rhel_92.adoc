...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | NetApp Linux主機公用程式CLI需要額外的程式庫套件相依性、才能支援Emulex HBA介面卡探索 | 在 RHEL 9.2 中、 NetApp Linux SAN 主機公用程式 CLI `sanlun fcp show adapter -v` 失敗、因為找不到支援HBA探索的程式庫套件相依性。 
