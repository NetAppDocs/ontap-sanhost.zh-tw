...
[NOTE]
====
安裝 Windows Host Utilities 7.2 後，您需要變更 LimTransferSize 參數值。請參閱 <<parameter-72,NVME參數>>。
...
[[parameter-72]]Windows Host Utilities 安裝程式會根據您在安裝期間所做的選擇，自動設定 Windows 登錄值以及 iSCSI 和 NVMe HBA 參數。請檢查這些值和作業系統版本。
