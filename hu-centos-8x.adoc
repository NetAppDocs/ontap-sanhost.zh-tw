...
summary: 在 CentOS 8.x 主機上安裝 Linux 主機實用程序，以管理與ONTAP LUN 的 FCP 和 iSCSI 協定操作。 
---
= 配置 CentOS 8.x 以支援 FCP 和 iSCSI 以用於ONTAP存儲
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 軟體為連接到ONTAP儲存的 Linux 主機提供管理和診斷工具。在 CentOS 8.x 主機上安裝 Linux 主機公用程式後，您可以使用主機公用程式來協助您管理ONTAP LUN 的 FCP 和 iSCSI 協定操作。
...
您可以使用 CentOS 8.x 的多路徑功能來管理ONTAP LUN。
...
有關 CentOS Red Hat 相容核心的已知問題，請參閱 Red Hat Enterprise Linux（RHEL）8.x 系列的link:hu-rhel-8x.html#step-7-review-the-known-issues["已知問題"]。
...
* 了解 CentOS Linux 虛擬化 (KVM)
+
CentOS Linux 可以作為 KVM 主機。這樣，您就可以使用基於 Linux 核心的虛擬機器 (KVM) 技術在單一實體伺服器上執行多個虛擬機器。KVM 主機不需要對ONTAP LUN 進行明確主機設定設定。
