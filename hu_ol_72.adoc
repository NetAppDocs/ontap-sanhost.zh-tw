...
Oracle Linux 7.2 作業系統的編譯是為了識別 ONTAP LUN 、並自動正確設定所有組態參數。該 `multipath.conf`檔案必須存在、多重路徑常駐程式才能啟動。如果此檔案不存在、您可以使用下列命令來建立空的零位元組檔案：
...
NOTE: 若要設定 Oracle Linux 7.2 Red Hat Enterprise 核心（ RHCK ）、請使用link:hu_rhel_72.html#recommended-settings["建議設定"]適用於 Red Hat Enterprise Linux （ RHEL ） 7.2 的。



== 已知問題

使用 ONTAP 版本的 Oracle Linux 7.2 沒有已知問題。
