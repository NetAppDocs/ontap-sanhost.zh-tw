...
[role="lead"]
您可以在執行 Oracle Linux 7.9 和 ONTAP 的啟動器主機上、將 NVMe over Fibre Channel （ NVMe / FC ）設定為目標。



== 支援能力

從 ONTAP 9.6 開始， Oracle Linux 7.9 支援 NVMe / FC 。Oracle Linux 7.9 主機可透過相同的光纖通道（ FC ）啟動器介面卡連接埠，同時執行 NVMe 和 SCSI 流量。如需支援的 FC 介面卡和控制器清單、請參閱 link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] 。如需支援組態的目前清單，請參閱link:https://mysupport.netapp.com/matrix/["互通性對照表工具"^]。
...
* NVMe / FC 不支援 sanlun ，因此 Oracle Linux 7.9 不支援 NVMe / FC 的 Linux 主機公用程式。使用ONTAP NetApp外掛程式隨附於原生NVMe CLI中的支援功能之一、即可取得的支援功能。
