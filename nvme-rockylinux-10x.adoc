...
summary: 如何為 Rocky Linux 10.x 設定帶有ONTAP儲存的 NVMe-oF 主機 
---
= 配置 Rocky Linux 10.x 以支援 NVMe-oF 和ONTAP存儲
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 Rocky Linux 10.x 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支援與功能"]。

NVMe-oF 與 Rocky Linux 10.x 有以下已知限制：
...
.步驟
. 在伺服器上安裝 Rocky Linux 10.x。安裝完成後，請確認您執行的是所需的 Rocky Linux 10.x 核心：
+
[source, cli]
----
uname -r
...
Rocky Linux 10.x 主機和ONTAP控制器之間透過 NVMe/TCP 支援安全的帶內身份驗證。
...
cat /etc/nvme/hostnqn
----
. 為 Rocky Linux 10.x 主機產生 dhchap 金鑰。
+
