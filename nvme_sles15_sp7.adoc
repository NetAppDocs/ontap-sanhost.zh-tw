...
[role="lead"]
SUSE Linux Enterprise Server 主機支援具有非對稱命名空間存取 (ANA) 的 NVMe over Fibre Channel (NVMe/FC) 和 NVMe over TCP (NVMe/TCP) 協定。  ANA 提供與 iSCSI 和 FCP 環境中的非對稱邏輯單元存取 (ALUA) 等效的多路徑功能。

.關於這項工作
您可以將以下支援和功能與 SUSE Linux Enterprise Server 15 SP7 的 NVMe-oF 主機配置一起使用。在開始配置過程之前，您還應該查看已知的限制。
...
** 從ONTAP 9.12.1 開始，引入了對 NVMe/TCP 和 NVMe/FC 安全帶內身份驗證的支援。您可以使用 SUSE Linux Enterprise Server 15 SP7 對 NVMe/TCP 和 NVMe/FC 進行安全帶內驗證。
** 支援使用唯一發現 NQN 的持久發現控制器 (PDC)。
** NVMe/TCP 的 TLS 1.3 加密支援。
** NetApp `sanlun` SUSE Linux Enterprise Server15 SP7 主機上的 NVMe-oF 不支援主機實用程式。相反，您可以依賴本機中包含的NetApp插件 `nvme-cli`適用於所有 NVMe-oF 傳輸的套件。
+
...
使用下列步驟驗證最低支援的 SUSE Linux Enterprise Server 15 SP7 軟體版本。

.步驟
. 在伺服器上安裝SUSE Linux Enterprise Server15 SP7。安裝完成後，請驗證您是否正在執行指定的 SUSE Linux Enterprise Server 15 SP7 核心：
+
[source, cli]
----
uname -r
----
+
以下範例顯示了 SUSE Linux Enterprise Server 核心版本：
...
使用 SUSE Linux Enterprise Server 15 SP7， `nvmefc-boot-connections.service`和 `nvmf-autoconnect.service`NVMe/FC 中包含的啟動服務 `nvme-cli`軟體包將在系統啟動時自動啟用。系統啟動完成後，驗證啟動服務是否已啟用。
...
NOTE: 從 SUSE Linux Enterprise Server 15 SP6 開始，NVMe/TCP 的預設設定 `ctrl-loss-tmo`超時已關閉。這意味著重試次數沒有限制（無限重試），並且您不需要手動配置特定的 `ctrl-loss-tmo`使用時的超時持續時間 `nvme connect`或者 `nvme connect-all`命令（選項 `-l`）。此外，NVMe/TCP 控制器在發生路徑故障時不會逾時，並會無限期地保持連線。
...
從ONTAP 9.11.1 開始，您可以為 SUSE Linux Enterprise Server 15 SP7 主機建立持久性發現控制器 (PDC)。需要 PDC 來自動偵測 NVMe 子系統的新增或刪除操作以及發現日誌頁面資料的變更。
