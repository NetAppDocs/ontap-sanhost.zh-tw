...
summary: 配置 Citrix Hypervisor 8.2 以實現多路徑，並使用特定參數和設定來針對ONTAP儲存的 FCP 和 iSCSI 協定操作。 
---
= 使用 ONTAP 儲存為 FCP 和 iSCSI 設定 Citrix Hypervisor 8.2
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
配置 Citrix Hypervisor 8.2 以實現多路徑，並使用特定參數和設定來針對ONTAP儲存的 FCP 和 iSCSI 協定操作。

[NOTE]
====
Linux Host Utilities 軟體包不支援 Citrix Hypervisor 作業系統。
...
您可以使用 Citrix Hypervisor 8.2 的多路徑來管理 ONTAP LUN。
