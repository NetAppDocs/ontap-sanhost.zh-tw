...
summary: 介紹如何將 Citrix Hypervisor 8.2 與 ONTAP 結合使用 
---
...
[role="lead"]
Linux 主機公用程式軟體可為連接至 ONTAP 儲存設備的 Linux 主機提供管理與診斷工具。在 Citrix Hypervisor 8.2 主機上安裝 Linux Host Utilities 時，您可以使用 Host Utilities 來協助您管理 ONTAP LUN 的 FCP 和 iSCSI 協定操作。
...
.步驟
. 驗證 `/etc/multipath.conf`文件存在。如果檔案不存在，請建立空白的零位元組檔案：
+
[source, cli]
----
