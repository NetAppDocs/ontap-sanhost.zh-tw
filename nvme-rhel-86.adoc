...
---
= 使用ONTAP儲存為 NVMe-oF 設定 RHEL 8.6
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 RHEL 8.6 設定 NVMe over Fabrics (NVMe-oF) 主機。有關更多支援和功能信息，請參閱link:hu-nvme-index.html["NVME-oF 概述"^]。

*RHEL 8.6 的 NVMe-oF 有以下已知限制：*

* 目前不支援使用 NVMe-oF 協定的 SAN 啟動。
* 預設情況下，RHEL 8.6 中的 NVMe-oF 主機上會停用核心 NVMe 多路徑；您必須手動啟用它。
...
.步驟
. 在伺服器上安裝 RHEL 8.6。安裝完成後，請驗證您是否正在執行所需的 RHEL 8.6 核心：
+
[source, cli]
----
uname -r
...
----
. 在 RHEL 8.6 主機上，檢查 `hostnqn`字串位於 `/etc/nvme/hostnqn`：
+
[source, cli]
----
...
NOTE: 如果 `hostnqn`字串不匹配，您應該使用 `vserver modify`命令來更新 `hostnqn`對應ONTAP儲存系統 NVMe 子系統上的字串以匹配 `hostnqn`細繩 `/etc/nvme/hostnqn`在主機上。
