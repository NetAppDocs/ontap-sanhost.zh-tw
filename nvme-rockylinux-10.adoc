...
summary: 如何使用 ONTAP 儲存為 Rocky Linux 10 設定 NVMe-oF 主機 
---
= 使用 ONTAP 儲存為 NVMe-oF 設定 Rocky Linux 10
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
.關於這項工作
您可以將以下支援和功能與 Rocky Linux 10 的 NVMe-oF 主機配置一起使用。在開始設定程序之前，您也應該先檢閱已知的限制。
...
** 從 ONTAP 9.12.1 開始，引入了對 NVMe/TCP 的安全帶內身份驗證的支援。您可以使用 Rocky Linux 10 對 NVMe/TCP 進行安全帶內身份驗證。
+
...
* 可用功能：
+
** 從 Rocky Linux 10 開始，本機 NVMe 多路徑始終處於啟用狀態，且不支援 NVMe-oF 的 DM 多路徑支援。
...
使用以下步驟驗證最低支援的 Rocky Linux 10 軟體版本。

.步驟
. 在伺服器上安裝 Rocky Linux 10。安裝完成後，請驗證您是否正在執行指定的 Rocky Linux 10 核心：
+
[source, cli]
----
uname -r
...
借助 Rocky Linux 10，  `nvmefc-boot-connections.service`和 `nvmf-autoconnect.service` NVMe/FC 中包含的啟動服務 `nvme-cli`系統啟動時，這些軟體包會自動啟用。啟動完成後，請驗證這些服務是否已啟用。
...
NOTE: 從 Rocky Linux 9.5 開始，NVMe/TCP 的預設設置 `ctrl_loss_tmo`超時已關閉。這表示重試次數沒有限制（無限期重試）。因此，使用或 `nvme connect-all`命令（選項 -l ）時不需要手動配置特定的 `ctrl_loss_tmo`超時持續時間 `nvme connect`。在這種預設行為下、當路徑故障時、 NVMe / TCP 控制器不會發生逾時、而且會無限期保持連線。
...
從 ONTAP 9.12.1 開始，Rocky Linux 10 主機和 ONTAP 控制器之間透過 NVMe/TCP 支援安全帶內身份驗證。
...
cat /etc/nvme/hostnqn
----
. 為 Rocky Linux 10 主機產生 dhchap 金鑰。
+
