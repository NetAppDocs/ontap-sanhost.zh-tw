...
summary: 如何使用ONTAP儲存為 Rocky Linux 10.0 設定 NVMe-oF 主機 
---
= 使用ONTAP儲存為 NVMe-oF 設定 Rocky Linux 10.0
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 Rocky Linux 10.0 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支援與功能"]。

*帶有 Rocky Linux 10.0 的 NVMe-oF 具有以下已知限制：*
...
.步驟
. 在伺服器上安裝 Rocky Linux 10.0。安裝完成後，請驗證您是否正在執行所需的 Rocky Linux 10.0 核心：
+
[source, cli]
----
uname -r
...
從ONTAP 9.12.1 開始，Rocky Linux 10.0 主機和ONTAP控制器之間透過 NVMe/TCP 支援安全帶內身份驗證。
...
cat /etc/nvme/hostnqn
----
. 為 Rocky Linux 10.0 主機產生 dhchap 金鑰。
+
