...
---
= 使用Red Hat Enterprise Linux 9.1搭配NetApp ONTAP 功能
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
NetApp強烈建議安裝Linux Unified Host Utilities、但並非強制性。公用程式不會變更Linux主機上的任何設定。這些公用程式可改善管理、並協助NetApp客戶支援部門收集您的組態相關資訊。
...
| 1508554 | NetApp Linux主機公用程式CLI需要額外的程式庫套件相依性、才能支援Emulex HBA介面卡探索 | 在RHEL 9.1中、NetApp Linux SAN主機公用程式CLI `sanlun fcp show adapter -v` 失敗、因為找不到支援HBA探索的程式庫套件相依性。 | 不適用 
|===
