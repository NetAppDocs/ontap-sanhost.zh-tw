...
.您需要的產品
如果您目前已安裝Linux Unified Host Utilities版本、則應升級或移除該版本、並使用下列步驟安裝最新版本。
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | NetApp Linux主機公用程式CLI需要額外的程式庫套件相依性、才能支援Emulex HBA介面卡探索 | 在RHEL 9.1中、NetApp Linux SAN主機公用程式CLI `sanlun fcp show adapter -v` 失敗、因為找不到支援HBA探索的程式庫套件相依性。 | 不適用 
|===