...
---
= 使用Red Hat Enterprise Linux 8.7搭配NetApp ONTAP 功能
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
RHEL 8.7作業系統經過編譯、可辨識ONTAP 出各種不完整的LUN、並自動針對ASA 各種不適用的ASA組態、正確設定所有組態參數。
...
| 1501048 | FC傳輸層逾時參數 `dev_loss_tmo` 對於遠端SCSI目標連接埠、會報告負值（-1） |  `dev_loss_tmo` timeout參數表示FC傳輸應隔離以防止遠端連接埠遺失的預設秒數。超過此值時、SCSI目標會從主機移除。此參數值應為正整數值。然而在RHEL 8.7中、 `sysfs` 檔案系統會傳回的負值（-1） `dev_loss_tmo` SCSI裝置的逾時參數。以下是驗證的命令列範例 `dev_loss_tmo` 工作主機上的參數：
`# cat /sys/devices/pci*/*/*/host*/rport*/fc_remote_ports/rport*/dev_loss_tmo
4294967295`。RHEL 8.7中沒有此問題的因應措施。 | link:https://bugzilla.redhat.com/show_bug.cgi?id=2124297["2124297"^] 
|===
