...
[role="lead"]
Rocky Linux 9.4 主機支援具有非對稱命名空間存取 (ANA) 的 NVMe/FC 和 NVMe/TCP 協定。 ANA相當於 iSCSI 和 FCP 環境中的非對稱邏輯單元存取 (ALUA) 多路徑，並使用核心 NVMe 多路徑功能實現。

如需支援組態的其他詳細資訊，請參閱link:https://mysupport.netapp.com/matrix/["互通性對照表工具"^]。

.關於這項工作
您可以將以下支援和功能與 Rocky Linux 9.4 的 NVMe-oF 主機配置一起使用。在開始設定程序之前，您也應該先檢閱已知的限制。
...
* 可用功能：
+
** Rocky Linux 9.4 預設為 NVMe 命名空間啟用核心 NVMe 多路徑，無需明確設定。
...
使用以下步驟驗證最低支援的 Rocky Linux 9.4 軟體版本。
