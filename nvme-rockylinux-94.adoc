...
summary: 如何使用 ONTAP 為 Rocky Linux 9.4 設定 NVMe-oF 主機 
---
= 使用 NVMe-oF 為 ONTAP 儲存配置 Rocky Linux 9.4
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
了解如何為 Rocky Linux 9.4 設定 NVMe over Fabrics (NVMe-oF) 主機。如需更多支援和功能信息，請參閱 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAP支援與功能"]。

帶有 Rocky Linux 9.4 的 NVMe-oF 沒有已知的限制。
...
.步驟
. 在伺服器上安裝 Rocky Linux 9.4。安裝完成後，驗證您執行的是指定的 Rocky Linux 9.4 核心：
+
[source, cli]
----
uname -r
