...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 Oracle Linux 6.5 與 ONTAP 設定為目標。
...
NOTE: 若要設定 Oracle Linux 6.5 Red Hat 相容核心（ RHCK ），請使用link:hu_rhel_65.html#recommended-settings["建議設定"]適用於 Red Hat Enterprise Linux （ RHEL ） 6.5 的。



== 已知問題

使用 ONTAP 版本的 Oracle Linux 6.5 沒有已知問題。


NOTE: 如需 Oracle Linux RHCK 已知問題，請參閱link:hu_rhel_65.html#known-problems-and-limitations["已知問題"]適用於 RHEL 6.5 的。
