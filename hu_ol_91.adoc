...
---
= 搭配 NetApp ONTAP 使用 Oracle Linux 9.1
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
Oracle Linux 9.1 作業系統是為了識別 ONTAP LUN 、並自動為 ASA 和非 ASA 組態正確設定所有組態參數。您可以使用下列建議設定、進一步最佳化主機組態的效能。
...
|===
.範例
下列範例示範如何修正覆寫的預設值。在此案例中 `multipath.conf` 檔案定義的值 `path_checker` 和 `no_path_retry` 與ONTAP 不兼容的如果因為主機仍連接其他SAN陣列而無法移除、則可針對ONTAP 具有裝置例項的LUN、特別修正這些參數。
...
NOTE: 若要設定 Oracle Linux 9.1 RedHat Enterprise Kernel （ RHCK ）、請使用 link:hu_rhel_91.html#recommended-settings["建議設定"] 適用於 Red Hat Enterprise Linux （ RHEL ） 9.1 。
