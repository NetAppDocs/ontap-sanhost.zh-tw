...
summary: 說明如何搭配ONTAP 使用CentOS 6.9搭配使用功能 
---
= 將 CentOS 6.9 與 ONTAP 搭配使用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
您可以使用 ONTAP SAN 主機組態設定、將 CentOS 6.9 設定為 ONTAP 作為目標。
...
對於CentOS 6.9"/etc/multipath.conf檔案必須存在、但您不需要對檔案進行特定變更。CentOS 6.9是以所有必要設定來編譯、以辨識及正確管理ONTAP 各個LUN。
...
....
. 使用「mkitrd」命令重新建立initrd-image。CentOS 6x及更新版本可使用下列命令：「mkitrd -f /boot/ initrd-"uname -r"。imguname -r"或命令：「Dracut -f"
. 重新啟動主機。
...
CentOS 6.9作業系統經過編譯、可辨識ONTAP 出各種無法辨識的LUN、並自動針對ASA 各種版本的支援和非ASA組態、正確設定所有組態參數。
...
如需CentOS（Red Hat相容核心）已知問題、請參閱 link:hu_rhel_69.html#known-problems-and-limitations["已知問題"] 適用於Red Hat Enterprise Linux（RHEL）6.9。
