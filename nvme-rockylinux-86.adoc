...
summary: 如何使用 ONTAP 為 Rocky Linux 8.6 設定 NVMe-oF 主機 
---
= 為 ONTAP 儲存配置帶有 NVMe-oF 的 Rocky Linux 8.6
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 8.6 主機支援具有非對稱命名空間存取 (ANA) 的 NVMe/FC 和 NVMe/TCP 協定。 ANA相當於 iSCSI 和 FCP 環境中的非對稱邏輯單元存取 (ALUA) 多路徑，並使用核心 NVMe 多路徑功能實現。

如需支援組態的其他詳細資訊，請參閱link:https://mysupport.netapp.com/matrix/["互通性對照表工具"^]。

.關於這項工作
您可以將以下支援和功能與 Rocky Linux 8.6 的 NVMe-oF 主機配置一起使用。在開始設定程序之前，您也應該先檢閱已知的限制。
...
* 已知限制：
+
** 對於 Rocky Linux 8.6 NVMe-oF 主機，核心 NVMe 多路徑預設為停用。因此、您需要手動啟用。
** 在 Rocky Linux 8.6 主機上，由於存在未解決的問題，NVMe/TCP 是一項技術預覽功能。如需詳細資訊、請參閱 https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/8.6_release_notes/index#technology-preview_file-systems-and-storage["Rocky Linux 8.6 發行說明"^] 。
...
使用以下步驟驗證最低支援的 Rocky Linux 8.6 軟體版本。

.步驟
. 在伺服器上安裝 Rocky Linux 8.6。安裝完成後，驗證您執行的是指定的 Rocky Linux 8.6 核心：
+
[source, cli]
----
uname -r
...
具有 ONTAP 的 Rocky Linux 8.6 的 NVMe-oF 主機配置有以下已知問題：
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Rocky Linux 8.6 NVMe-oF 主機建立重複的持久發現控制器 | 在NVMe over Fabrics（NVMe）主機上、您可以使用「NVMe Discover-p」命令來建立持續探索控制器（PD）。使用此命令時、每個啟動器目標組合只能建立一個PDC。但是，如果您在 NVMe-oF 主機上執行 ONTAP 9.10.1 和 Rocky Linux 8.6，則每次執行「nvme discover -p」時都會建立重複的 PDC。這會導致主機和目標上的資源使用不必要。 
|===