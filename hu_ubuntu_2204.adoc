...
summary: 在 Ubuntu 22.04 主機上安裝 Linux Host Utilities，以管理 ONTAP LUN 的 FCP 和 iSCSI 協定操作。 
---
= 使用ONTAP儲存為 FCP 和 iSCSI 配置 Ubuntu 22.04
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilities 軟體為連接到ONTAP儲存的 Linux 主機提供管理和診斷工具。在 Ubuntu 22.04 主機上安裝 Linux Host Utilities 時，您可以使用 Host Utilities 來協助您管理ONTAP LUN 的 FCP 和 iSCSI 協定操作。
...
您可以使用 Ubuntu 22.04 的多路徑來管理ONTAP LUN。
